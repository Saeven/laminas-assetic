...
use Circlical\AsseticBundle\View\StrategyInterface;
        ...
        foreach ($filters as $alias => $options) {
            ...
            if (!$fm->has($filterId)) {
                if (is_array($option) && !empty($option)) {
                    ...
                    $filter = (new ReflectionClass($name))->newInstanceArgs($option);
                } elseif ($option) {
                    /** @var FilterInterface $filter */
        ...
        if (!isset($this->strategy[$rendererName])) {
            ...
            if (!$instance instanceof StrategyInterface) {
                throw new Exception\DomainException(
                    sprintf(
                        'strategy class "%s" is not instanceof "Circlical\AsseticBundle\View\StrategyInterface"',
                        $strategyClass
                    )
                );
            }
